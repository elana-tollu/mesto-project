...
export function initEditProfileForm() {
    //мод.окно редактирования профиля
...
function showEditProfile() {
    ...
    const currentUserName =
        profileInfo.querySelector(".profile__name").textContent; // присвоить переменной текстовое содержимое узла ДОМ
...
function saveEditProfile(submitEvent) {
    ...
    const newProfileDescription =
        profileForm.elements["user-description"].value; // найти инпуты, взять данные из инпутов
    updateUser({
        // обновить объект, присвоив ключам новые значения
        name: newUserName,
...
const buttonCancelAvatar = document.querySelector(
    ".popup__button-close_edit-avatar"
);
const avatarImage = document.querySelector(".profile__avatar"); // присвоить переменной текстовое содержимое узла ДОМ
const avatarForm = document.forms["edit-avatar"];
const saveAvatarButton = popupAvatar.querySelector(
    ".popup__button-save_edit-avatar"
);
